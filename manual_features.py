...
def get_features_from_seq(data,gene_index, files_list, gene_names, chroms, tss_centers, gene_coords , strands, gex):

    data.loc([gene_index],['gene_name'])=gene_names[gene_index]
    c =  gene_coords[gene_index]
    strand = (strands[gene_index]=='+').astype(int)
...
def build_dataset():
    ...
    for gene_index in tqdm(range(len(gene_names))):
        get_features_from_seq(data,gene_index, bw_files, gene_names, chroms, tss_centers, gene_coords , strands, gex)
